...
class DriverRequest(Event):
    ...
    def do(self, dispatcher, monitor):
        ...
        if rider is not None:
            drive_time = self.driver.start_ride(rider)
            if drive_time > rider.patience:
                events.append(Cancellation(self.timestamp + rider.patience, rider))
                events.append(Pickup(self.timestamp + drive_time, rider, self.driver))
            else:
                events.append(Pickup(self.timestamp + drive_time, rider, self.driver))
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if event_type == "DriverRequest":
                driver_name = tokens[2]
                location = deserialize_location(tokens[3])
                speed = int(tokens[4])
                driver = Driver(driver_name, location, speed)
                ...
                rider = Rider(rider_name, origin,destination, 'waiting', patience)
